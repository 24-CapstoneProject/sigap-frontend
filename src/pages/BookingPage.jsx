...
import { rooms, bookingHistory, allBookings } from "../data/mockData.js";
...
export default function BookingPage({ user }) {
  ...
  const [bookings, setBookings] = useState(user.role === "admin" ? allBookings : bookingHistory);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-900">
            {user.role === "admin" ? "Manajemen Peminjaman Ruang" : "Peminjaman Ruang Kelas SG"}
          </h2>
          <p className="text-sm text-gray-400 mt-0.5">
            {user.role === "admin" ? "Validasi pengajuan peminjaman dari mahasiswa" : "Ajukan peminjaman ruang untuk kegiatan perkuliahan"}
          </p>
        </div>
        {user.role === "mahasiswa" && (
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 py-16 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {user.role === "mahasiswa" ? "Klik tombol di atas untuk mengajukan peminjaman." : "Belum ada pengajuan masuk."}
          </p>
        </div>
      ) : (
